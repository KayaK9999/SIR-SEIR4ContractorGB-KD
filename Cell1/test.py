...
if __name__ == '__main__':
    ...
    for i in range(0, length):
        ...
        for j in range(0, width):
            ...
            tmp.append({'value': 0, 'g': random.gauss(0.7, 0.01), 's': random.gauss(0.7, 0.01), 'b1': b/100, 'b2': random.randint(0, 100-b)/100, 'count': 0, 'y': 0, 'keep': 0})
        ...
        for j in range(0, width):
            ...
            if (isCal != 0):
                ...
                if (initMatrix[(i + 1) % width][(j + 1) % width]['value'] == 2):
                    isCal = 1
                    n = n + 1
                    sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 1) % width][(j + 1) % width]['s'] / 2)
                if (initMatrix[(i + length - 1) % width][(j + 1) % width]['value'] == 2):
                    isCal = 1
                    n = n + 1
                    sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 1) % width][(j + 1) % width]['s'] / 2)
                if (initMatrix[(i + 1) % width][(j + length - 1) % width]['value'] == 2):
                    isCal = 1
                    n = n + 1
                    sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 1) % width][(j + length - 1) % width]['s'] / 2)
                if (initMatrix[(i + length - 1) % width][(j + length - 1) % width]['value'] == 2):
                    isCal = 1
                    n = n + 1
                    sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 1) % width][(j + length - 1) % width]['s'] / 2)
                if (isCal != 0):
                    L1 = sumL1 / n
                else:
                    if (initMatrix[i][(j + 2) % width]['value'] == 2):
                    ...
                    if (isCal != 0):
                        ...
                        if (initMatrix[(i + 1) % width][(j + 2) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 1) % width][(j + 2) % width]['s'] / 5)
                        if (initMatrix[(i + length - 1) % width][(j + 2) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 1) % width][(j + 2) % width]['s'] / 5)
                        if (initMatrix[(i + 1) % width][(j + length - 2) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 1) % width][(j + length - 2) % width]['s'] / 5)
                        if (initMatrix[(i + length - 1) % width][(j + length - 2) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 1) % width][(j + length - 2) % width]['s'] / 5)
                        if (initMatrix[(i + 2) % width][(j + 1) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 2) % width][(j + 1) % width]['s'] / 5)
                        if (initMatrix[(i + length - 2) % width][(j + 1) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 2) % width][(j + 1) % width]['s'] / 5)
                        if (initMatrix[(i + 2) % width][(j + length - 1) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 2) % width][(j + length - 1) % width]['s'] / 5)
                        if (initMatrix[(i + length - 2) % width][(j + length - 1) % width]['value'] == 2):
                            isCal = 1
                            n = n + 1
                            sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 2) % width][(j + length - 1) % width]['s'] / 5)
                        if (isCal != 0):
                            L1 = sumL1 / n
                        else:
                            if (initMatrix[(i + 2) % width][(j + 2) % width]['value'] == 2):
                                isCal = 1
                                n = n + 1
                                sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 2) % width][(j + 2) % width]['s'] / 8)
                            if (initMatrix[(i + length - 2) % width][(j + 2) % width]['value'] == 2):
                                isCal = 1
                                n = n + 1
                                sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 2) % width][(j + 2) % width]['s'] / 8)
                            if (initMatrix[(i + 2) % width][(j + length - 2) % width]['value'] == 2):
                                isCal = 1
                                n = n + 1
                                sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + 2) % width][(j + length - 2) % width]['s'] / 8)
                            if (initMatrix[(i + length - 2) % width][(j + length - 2) % width]['value'] == 2):
                                isCal = 1
                                n = n + 1
                                sumL1 = sumL1 + math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + length - 2) % width][(j + length - 2) % width]['s'] / 8)
                            if (isCal != 0):
                                L1 = sumL1 / n
                            else:
                                L1 = 0
                ...
                for k2 in range(0, 5):
                    if(initMatrix[(i + k1 + length - 2)%length][(j + k2 + length - 2)%width]['value'] == 2):
                        if(initMatrix[(i + k1 + length - 2)%length][(j + k2 + length - 2)%width]['s'] > sMax):
                            ...
                            position = [k1 ,k2]
            if (sMax != 0 and (position[0] != 2 or position[1] != 2)):
                L2 = math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + k1 + length - 2)%length][(j + k2 + length - 2)%width]['s'] / (math.pow((position[0] - 2), 2) + math.pow((position[1] - 2), 2)))
            # 计算L3
            L3 = 0
            for k1 in range(0, 5):
                for k2 in range(0, 5):
                    if(initMatrix[(i + k1 + length - 2)%length][(j + k2 + length - 2)%width]['value'] == 2 and (k1 != 2 or k2 != 2)):
                        temp = math.sqrt(initMatrix[i][j]['g'] * initMatrix[(i + k1 + length - 2)%length][(j + k2 + length - 2)%width]['s'] / (math.pow((k1 - 2), 2) + math.pow((k2 - 2), 2)))
                        if(temp > L3):
                            L3 = temp
            # 计算a
            lList = [L1, L2, L3]
    ...
    plt.xlabel('t')
    plt.ylabel('i(t)')
    plt.plot(range(0, 30), zero, color='#f1eef6', label='1')
    plt.plot(range(0, 30), one, color='#bdc9e1', label='2')
    plt.plot(range(0, 30), two, color='#74a9cf', label='3')
    plt.plot(range(0, 30), three, color='#2b8cbe', label='4')
    plt.show()
